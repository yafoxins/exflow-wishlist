/**
 * Add Item Modal
 * Модальное окно для добавления подарка в список желаний
 */
...
export const AddItemModal: React.FC<AddItemModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Ошибка добавления подарка');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">Добавить подарок</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              ...
              fullWidth
            >
              {isLoading ? 'Добавление...' : 'Добавить подарок'}
            </Button>
          </div>
        </form>
      </div>
